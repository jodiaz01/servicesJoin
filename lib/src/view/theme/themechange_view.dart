  ...
  Widget build(BuildContext context) {
    ...
    return
      ...
      CustomAppbar(
        ...
        icon: Icon(Icons.light_mode_outlined, size: 38,color: themeprovider.currentTheme.secondaryHeaderColor,),
        child: Padding(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return ListTile(
                    ...
                    title: index == 0
                        ? const Text(
                            'Modo Claro',
                            style: TextStyle(fontWeight: FontWeight.bold,color: AppColors.primaryColor1),
                          )
                        : index == 1
                            ...
                            : index == 2
                                ? const Text('Cripsom claro',
                                    style:
                                        TextStyle(fontWeight: FontWeight.bold,color: AppColors.primaryColor3))
                                : index ==3 ? const Text('Modo Yankees',
                                    style:
                                        TextStyle(fontWeight: FontWeight.bold,color: AppColors.yankeeblue))
                        : index ==4 ? const Text('Modo Aguila',
                        style:   TextStyle(fontWeight: FontWeight.bold,color: AppAllColors.darkorange)) :SizedBox(),

                    leading:
                    index == 0 ? const Icon(FontAwesomeIcons.lightbulb,color: AppColors.primaryColor1)
                        : index == 1
                            ? const Icon(Icons.light_mode_outlined)
                            : index == 2
                                ? const Icon(Icons.sunny_snowing, color: AppColors.primaryColor3,)
                                : index == 3 ? const Icon(Icons.cloud_queue,color: AppColors.yankeeblue)
                                : index == 4 ?  const Icon(FontAwesomeIcons.crow,color: AppAllColors.darkorange) :SizedBox()
                  );
                })),
      ),
    );
