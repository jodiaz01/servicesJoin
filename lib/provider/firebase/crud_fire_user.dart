...
import 'package:services_joined/conexion/server.dart';
...
class FirebaseProvider extends ChangeNotifier {
  ...
  Future<void> IniSeccion(LoginFormProvider provider, context,
    ...
    try {
      ...
      if (data.docs.isNotEmpty) {
        ...
        // CustomAlert((context), "Datos Incorrectos",
        //     Text("Verifique usario Y contrasena "), "N/A", true);
      }
      notifyListeners();
    } catch (error) {
  ...
  iniciaData() async {
    ...
    //datos perfil completado
    try {
      allPelfiles.clear();

      final usuariomap = await Conexion.db.collection('users').get();
      //
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (availableBiometrics.contains(BiometricType.strong)) {
          // Obtener la huella digital capturada
          // String huella = '...Tiene face agregado';
          // Aquí debes utilizar la biblioteca o SDK del lector de huellas digitales para obtener la huella en el formato adecuado

            // if(createORead == "Create") {
            //   Guardar la huella digital en Firestore
            //   await _huellasCollection.add({
            //     'huella': huella,
            //   });
            // }
...
// Future<bool> EdidPerfilWithFiles(
//     List<File> filePath, String documents, context) async {
//   bool Ereturn = false;
//
//   for (File file in filePath) {
//     final Reference ref =
//         Conexion.storage.ref('imagenes').child(file.path.split('/').last);
//     final UploadTask uploadTask = ref.putFile(file);
//
//     // Utiliza un StreamBuilder para escuchar el progreso de subida
...
//           var totalprogress = (progress * 100).toStringAsFixed(2);
//
//           // Aquí puedes actualizar el porcentaje de subida o mostrar una notificación
//           print('Progreso de subida: ${(progress * 100).toStringAsFixed(2)}%');
//           ScaffoldMessenger.of(context).showSnackBar(SnackBar(
//             content: Text(totalprogress),
//             duration: Duration(seconds: 12),
//           ));
//         }
//
//         return SizedBox(); // Retorna un contenedor vacío, no afecta el UI final
//       },
//     );
//
...
//       });
//       Ereturn = true;
//       await getCuerrentUSer(documents);
//     } else {
//       Ereturn = false;
//     }
//   }
